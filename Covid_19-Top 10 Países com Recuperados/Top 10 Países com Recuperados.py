...
# Top 10 Países com Recuperados
top10_recovered = pd.DataFrame(data.groupby('Country')['Recovered'].sum().nlargest(10).sort_values(ascending = False))
fig1 = px.bar(top10_recovered, x = top10_recovered.index, y = 'Recovered', height = 600, color = 'Recovered', orientation= 'v',
            title = 'Top 10 Países com Recuperados', color_continuous_scale = px.colors.sequential.Viridis)
...
# data.groupby = usado para agrupar os dados, nesse caso ('Country')['Recovered']
...
# x = Dados que estarão no eixo x, nesse caso, os dados top10_recovered
# y = Dados que estarão no eixo y, nesse caso, os dados 'Recovered'
